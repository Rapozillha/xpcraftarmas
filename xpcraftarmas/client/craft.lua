        ...
        if ESX ~= nil and Crafting.Locations ~= nil then
            ...
            for i=1, #Crafting.Locations, 1 do
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Crafting.Locations[i].x, Crafting.Locations[i].y, Crafting.Locations[i].z, true) < 2.5 then
                    DrawText3D(Crafting.Locations[i].x, Crafting.Locations[i].y, Crafting.Locations[i].z, Config.Text['craft_item'])
...
RegisterNUICallback('CraftingSuccess', function()
    ...
    TriggerServerEvent("xpcraftarmas:CraftingSuccess", CurrentCraft)
end)
...
RegisterNUICallback('CraftingFailed', function()
    ...
    ClearPedTasks(GetPlayerPed(-1))
    FreezeEntityPosition(GetPlayerPed(-1),false)
    TriggerServerEvent("xpcraftarmas:CraftingFailed", CurrentCraft)
end)
...
function OpenCraftMenu()
    local elements = {}
    ESX.TriggerServerCallback('xpcraftarmas:GetSkillLevel', function(level)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'crafting_actions', {
            title    = "Craft de Armas",
            elements = elements,
            align    = 'top-left',
        }, function(data, menu)
            menu.close()
            CurrentCraft = data.current.value
            ESX.TriggerServerCallback('xpcraftarmas:HasTheItems', function(result)
                if result then
                    ...
                    exports['okokNotify']:Alert('CRAFT', Config.Text["no_material"], 1500, 'info', true)
                    --exports['mythic_notify']:DoHudText('inform', Config.Text["no_material"])
                end
            end, CurrentCraft)
